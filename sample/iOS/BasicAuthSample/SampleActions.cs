...
namespace BasicAuthSample
{
    public class SampleActions
    {
        ...
        //
        public static void PostMultiPartForm()
        {
            if (MAS.MASState != MASState.DidStart)
            {
                Alert("MAS.Invoke", "You must initialize the SDK before calling API");
            ...
            else
            {
                try
                {
                    ...
                    UIProgressView progress = new UIProgressView(UIProgressViewStyle.Default)
                    {
                    ...
                    picker.FinishedPickingMedia += (object sender, UIImagePickerMediaPickedEventArgs e) =>
                    {
                        ...
                        //UploadFormData(request, fileURL);
                        MAS.PostMultiPartForm(request, (formData) =>
                        {
                            NSError error = null;
                            formData.appendPartWithFileURL(fileURL, "adi.jpg", out error);
                        }, (prog) =>
                        {   
                            progress.Progress = (float)prog.FractionCompleted;
